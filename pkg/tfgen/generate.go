...
func (g *generator) gatherResource(rawname string,
	...
	if !isProvider {
		pd, err := getDocsForProvider(g.language, g.info.Name, ResourceDocs, rawname, info.Docs, g.info)
		if err != nil {
			return "", nil, err
		}
		parsedDocs = pd
...
func (g *generator) gatherDataSource(rawname string,
	...
	parsedDocs, err := getDocsForProvider(g.language, g.info.Name, DataSourceDocs, rawname, info.Docs, g.info)
	if err != nil {
		return "", nil, err
	}
