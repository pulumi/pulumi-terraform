...
func (nt *csharpNestedTypes) declareType(declarer declarer, namePrefix, name, nameSuffix string, typ *propertyType, isInput, isPlain bool) string {
	...
	if existing := nt.nameToType[typeName]; existing != nil {
		contract.Failf("Nested type %q declared by %s was already declared by %s", typeName, existing.declarer.Name(), declarer.Name())
	}
...
func (nt *csharpNestedTypes) gatherFromProperties(declarer declarer, namePrefix, nameSuffix string, ps []*variable, isInput, isPlain bool) {
	for _, p := range ps {
		nt.gatherFromPropertyType(declarer, namePrefix, p.name, nameSuffix, p.typ, isInput, isPlain)
	}
}

func (nt *csharpNestedTypes) gatherFromPropertyType(declarer declarer, namePrefix, name, nameSuffix string, typ *propertyType, isInput, isPlain bool) {
	switch typ.kind {
...
func (g *csharpGenerator) emitModuleMember(mod *module, member moduleMember, nested *csharpNestedTypes) (string, error) {
...
func (g *csharpGenerator) emitConfigVariable(w *tools.GenWriter, v *variable) {
	...
	w.Writefmtln("        public static %s%s %s { get; set; } = %s;", propertyType, nullableSigil, propertyName, initializer)
	w.Writefmtln("")
}
...
func (rg *csharpResourceGenerator) generateResourceClass() {
	...
	if !rg.res.IsProvider() {
		rg.w.Writefmtln("        /// <summary>")
		rg.w.Writefmtln("        /// Get an existing %s resource's state with the given name, ID, and optional extra", className)
...
func csharpType(typ *propertyType, wrapInput bool) string {
	return qualifiedCSharpType(typ, "", wrapInput)
}
