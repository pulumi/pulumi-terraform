...
func NewTerraformLogRedirector(ctx context.Context) *LogRedirector {
	logger := p.GetLogger(ctx)
	return &LogRedirector{
		writers: map[string]func(string) error{
			tfTracePrefix: func(msg string) error { logger.Debug(msg); return nil },
			tfDebugPrefix: func(msg string) error { logger.Debug(msg); return nil },
			tfInfoPrefix:  func(msg string) error { logger.Info(msg); return nil },
			tfWarnPrefix:  func(msg string) error { logger.Warning(msg); return nil },
			tfErrorPrefix: func(msg string) error { logger.Error(msg); return nil },
		},
	}
}
...
func (lr *LogRedirector) Write(p []byte) (n int, err error) {
	...
	for len(p) > 0 {
		...
		if !has || !lr.enabled {
			...
			contract.Assertf(w != nil, "No writer should be nil")
		}
		if err := w(s); err != nil {
			return written, err
		}
